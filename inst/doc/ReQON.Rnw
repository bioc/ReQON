...
non-reference (error) allele frequency greater than \texttt{nraf} (default = 0.05).  Second, we remove positions with more than \texttt{nerr} 
called errors (default = 2).  These thresholds set a maximum number of allowable called errors for positions with low coverage and a maximum 
frequency of non-reference (error) bases for positions with high coverage. 
